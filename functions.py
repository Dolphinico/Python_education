...
# модули отделяют блоки кода, от основной программы
# при помощи модулей, можно сохранить функцию в отдельный модуль и при необходимости импортировать его 

# пример создания модуля и его импортирование:
# текущую функцию мы добавляем в модуль pizza.py
# затем импортируем ее в новый файл making_pizza.py и там уже вызываем функцию.
...
# так выглядит первый способ импортирования, при котором записывается команда import с именем модуля,
# открывает доступ программе ко всем функциям из модуля: имя_модуля.имя_функции()
...
# разделяя функции запятой, можно вызвать любое количество
...
# имя  импортируемой функции может конфликтовать с именем существующей функцией
# либо имя слишком длинное, в таком случае проще использовать Alias(bind, псевдоним)
# в следующем примере  функции making_pizzas() псевдоним mp(), ключевое слово 'as' выполняет эту функцию

from pizza import making_pizzas as mp
...
# принцип такой же, только теперь псевдоним присваивается модулю

import pizza as p

p.making_pizzas(12,'sss')
p.making_pizzas(23,'sadsad','asdsadsa')
...
# Python можно приказать импортировать каждую функцию в модуле, для этого нужен оператор - *
...
# * в команде import приказывает Python скопировать каждую функцию
